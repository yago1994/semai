// background.js — patch loader service worker
...
function validateManifest(manifest) {
  ...
  return manifest.patches.filter((p) => {
    ...
    // Semver: minExtensionVersion must be <= current version
    if (p.minExtensionVersion && !semverSatisfies(p.minExtensionVersion)) {
      console.log(
        `[semai] Skipping patch ${p.id} (requires >= ${p.minExtensionVersion})`
      );
      return false;
    }
...
// Simple semver "major.minor.patch" — returns true if current >= required
